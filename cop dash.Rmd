...
movavgchart(ed_avondale, "Avondale", 321) |>
  layout(showlegend = FALSE)
```
...
reg_pat <- ggplot(reg, aes(x = Date, y = Registry)) +
...
reg$Visits <- sample(1:50000, nrow(reg))
reg$VisitRate <- reg$Visits/reg$Registry
...
reg_visit <- ggplot(reg, aes(x = Date, y = VisitRate)) +
  geom_line(linewidth = 1, color = cchmc_dark_blue) +
  labs(
    x = NULL, 
    y = "%", 
